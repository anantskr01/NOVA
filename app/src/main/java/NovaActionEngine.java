...
public final class NovaActionEngine {
    ...
    public boolean execute(String type, String value) {
        ...
        try {
            ...
            if (!validation.isEmpty()) { if (callback != null) callback.status("ACTION BLOCKED • " + validation); return false; }
            switch (action) {
                ...
                case "wait":
                    ...
                    SystemClock.sleep(Math.max(100L, Math.min(delay, 2500L))); return true;
                ...
                case "open_package":
                    Intent pkg = context.getPackageManager().getLaunchIntentForPackage(value.trim()); if (pkg == null) return false; launch(pkg); return true;
                case "open_app":
                    android.content.pm.ResolveInfo info = apps.resolve(value); Intent appIntent = apps.launchIntent(info); if (appIntent == null) return false; launch(appIntent); return true;
                case "settings": launch(new Intent(Settings.ACTION_SETTINGS)); return true;
                case "pc_read_file": case "pc_list_files": case "pc_write_file": case "pc_run_allowed":
                    String result = NovaPcRuntime.execute(action, value);
                    if (callback != null) callback.status("PC • " + action + " • COMPLETE");
                    return result != null;
        ...
        } catch (Exception e) { if (callback != null) callback.status("ACTION FAILED • " + action + " • " + (e.getMessage() == null ? "error" : e.getMessage())); return false; }
    }

    private boolean typeText(String value) {
        GestureAccessibilityService service = GestureAccessibilityService.getInstance();
        if (service == null) { if (callback != null) callback.status("ACCESSIBILITY NOT CONNECTED"); return false; }
    ...
    private boolean swipe(String direction) {
        GestureAccessibilityService service = GestureAccessibilityService.getInstance();
        if (service == null) { if (callback != null) callback.status("ACCESSIBILITY NOT CONNECTED"); return false; }
        if ("up".equals(direction)) service.swipeUp(); else if ("down".equals(direction)) service.swipeDown(); else if ("left".equals(direction)) service.swipeLeft(); else service.swipeRight(); return true;
    }
    private boolean openWebUrl(String raw) {
        if (raw == null || raw.trim().isEmpty()) return false;
        Uri uri = Uri.parse(raw.trim()).normalizeScheme(); String scheme = uri.getScheme();
        if (!("http".equals(scheme) || "https".equals(scheme))) { if (callback != null) callback.status("ACTION BLOCKED • URL SCHEME"); return false; }
        Intent intent = new Intent(Intent.ACTION_VIEW, uri);
        if (intent.resolveActivity(context.getPackageManager()) == null) { if (callback != null) callback.status("ACTION FAILED • NO BROWSER"); return false; }

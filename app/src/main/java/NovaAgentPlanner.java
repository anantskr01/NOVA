...
public final class NovaAgentPlanner {
    ...
    public boolean execute(String rawPlan) {
        ...
        try {
            ...
            if (actions == null) { if (!say.isEmpty()) listener.reply(say); lastExecutionSuccessful = true; return true; }
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (!registry.validate(type, value)) { lastFailures.add(type.isEmpty() ? "unknown" : type); listener.status("AGENT • BLOCKED INVALID TOOL"); continue; }
                if (registry.requiresConfirmation(type)) { lastFailures.add(type); listener.status("AGENT • CONFIRMATION REQUIRED • " + type); continue; }
                ...
                if (!ok && !type.contains(".") && MAX_RETRIES > 0) ok = executeOne(type, value);
            ...
            return true;
        } catch (Exception e) { Log.e(TAG, "PLAN EXECUTION ERROR", e); return false; }
    }
    ...
    private boolean executeOne(String type, String value) {
        try {
            if ("read_screen".equals(type)) { String text = executor.readScreen(); listener.status(text == null ? "SCREEN • NO READABLE TEXT" : text); return text != null; }
            ...
            if ("open_url".equals(type)) { String lower = value.toLowerCase(Locale.ROOT); if (value.isEmpty() || !(lower.startsWith("https://") || lower.startsWith("http://"))) return false; }

...
public final class NovaAgentPlanner {
    ...
    private static final int MAX_STEPS=8, MAX_RETRIES=1, MAX_TOOL_OUTPUT_CHARS=12000;
    private final ActionExecutor executor; private final Listener listener; private final NovaToolRegistry registry;
    private final List<String> lastFailures=new ArrayList<>(); private final List<String> lastStepResults=new ArrayList<>();
    ...
    public boolean execute(String rawPlan){
        ...
        try {
            JSONObject plan=parseObject(rawPlan); if(plan==null)return false;
            String say=plan.optString("say","").trim(); lastPlanClaimedComplete=plan.optBoolean("complete",false);
            ...
            int requested=actions.length(),count=Math.min(requested,MAX_STEPS);
            if(requested>MAX_STEPS){lastFailures.add("plan_too_large");listener.status("AGENT • PLAN LIMITED TO "+MAX_STEPS+" STEPS");}
            listener.status("AGENT • EXECUTING "+count+" STEPS"); boolean[] succeeded=new boolean[count];
            for(int i=0;i<count;i++){
                JSONObject action=actions.optJSONObject(i); String label="STEP "+(i+1);
                if(action==null){lastFailures.add("invalid_action_"+(i+1));lastStepResults.add(label+" • FAILED • invalid action");continue;}
                int required=action.optInt("requires",0);
                if(required>0&&(required>i||!succeeded[required-1])){lastFailures.add("dependency_blocked_step_"+(i+1));lastStepResults.add(label+" • BLOCKED • requires STEP "+required);continue;}
                ...
                boolean external=requiresExternalTool(type); boolean ok=external?executeExternalTool(type,value):executeOne(type,value);
                if(!ok&&!external&&MAX_RETRIES>0){listener.status("AGENT • RETRY • "+type);ok=executeOne(type,value);}
            ...
            lastExecutionSuccessful=lastFailures.isEmpty()&&(count>0||lastPlanClaimedComplete); return true;
        }catch(Exception e){Log.e(TAG,"PLAN EXECUTION ERROR",e);lastFailures.add("planner_error");return false;}
    }
    private boolean requiresExternalTool(String type){return "type_text".equals(type)||"wait".equals(type)||type.contains(".");}

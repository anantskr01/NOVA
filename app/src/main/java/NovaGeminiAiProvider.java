...
/** Gemini API provider. API key is supplied by NOVA's existing secure configuration. */
    ...
    public void chat(String endpoint, String apiKey, String model, JSONArray messages, Callback callback) {
        executor.execute(() -> {
            ...
            try {
                ...
                String selectedModel = model == null || model.trim().isEmpty() || model.startsWith("gpt-")
                        ? DEFAULT_MODEL : model.trim();
                connection = (HttpURLConnection) new URL(BASE_URL + selectedModel + ":generateContent").openConnection();
                ...
                if (messages != null) {
                    for (int i = 0; i < messages.length(); i++) {
                        ...
                        String role = "assistant".equals(message.optString("role", "user")) ? "model" : "user";
                        contents.put(new JSONObject().put("role", role)

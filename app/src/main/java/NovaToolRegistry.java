...
public final class NovaToolRegistry {
    ...
    public NovaToolRegistry() {
        ...
        add("calculate", "Evaluate a safe arithmetic expression deterministically", true);
    ...
    public synchronized void register(NovaTool tool) {
        ...
        tools.put(tool.type().trim().toLowerCase(Locale.ROOT), tool);
    }
    ...
    public synchronized String promptSummary() {
        ...
        for (NovaTool t : tools.values()) {
            ...
            out.append("- ").append(t.type()).append(": ").append(t.description())
                    .append("; reversible=").append(t.reversible()).append('\n');
        }

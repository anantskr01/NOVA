...
public final class NovaBrain {
    ...
    public NovaBrain(Context c, NovaActionEngine a, NovaMemory m, Listener l) {
        ...
        planner = new NovaAgentPlanner(new NovaAgentPlanner.ActionExecutor() {
            @Override public boolean execute(String t, String v) { return NovaBrain.this.actions != null && NovaBrain.this.actions.execute(t, v); }
            @Override public String readScreen() { GestureAccessibilityService s = GestureAccessibilityService.getInstance(); return s == null ? "Accessibility service is not connected." : s.getVisibleTextSummary(); }
            @Override public String readUiState() { return getUiSnapshot(); }
            @Override public String activePackageName() { GestureAccessibilityService s = GestureAccessibilityService.getInstance(); return s == null ? "" : s.getActivePackageName(); }
            @Override public boolean clickText(String t) { GestureAccessibilityService s = GestureAccessibilityService.getInstance(); return s != null && s.clickText(t); }
            @Override public boolean clickVisibleIndex(int i) { GestureAccessibilityService s = GestureAccessibilityService.getInstance(); return s != null && s.clickVisibleIndex(i); }
            @Override public String executeTool(String t, String v) { return executeIntelligenceTool(t, v); }
            @Override public String executeParallel(String v) { return executeParallelTools(v); }
        }, new NovaAgentPlanner.Listener() {
            @Override public void status(String t) { NovaBrain.this.status(t); }
            @Override public void reply(String t) { NovaBrain.this.reply(t); }
        }, tools);
    }
    ...
    /** Cancels only goals waiting in NOVA's brain queue; the active goal keeps running. */
    public synchronized void cancelQueuedGoals() {
        int count = queue.size();
        queue.clear();
        status("BRAIN • QUEUED GOALS CANCELLED • " + count);
    }
    ...
    private String executeParallelTools(String value) {
        try {
            ...
            for (int i = 0; i < steps.length(); i++) {
                ...
                if (s == null) return "{\"ok\":false,\"error\":\"parallel_invalid_step\"}";
    ...
    private JSONArray toOrchestratorSteps(JSONArray source) throws Exception {
        ...
        for (int i = 0; i < source.length(); i++) { JSONObject s = source.optJSONObject(i); if (s != null) out.put(new JSONObject().put("id", s.optString("id", String.valueOf(i))).put("tool", s.optString("type", "")).put("input", new JSONObject().put("value", s.optString("value", "")))); }
        return out;
    }

    private JSONObject executeToolJson(String tool, JSONObject input) {
        try { return new JSONObject(executeIntelligenceTool(tool, input == null ? "" : input.optString("value", ""))); }
        catch (Exception e) { try { return new JSONObject().put("ok", false).put("error", "tool_failed"); } catch (Exception ignored) { return new JSONObject(); } }
    }
    ...
    private String getModel() { return context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).getString(MODEL, "gemini-3.8-flash").trim(); }
    ...
    public void shutdown() {
        ...
        orchestrator.shutdown(); agentExecutor.shutdownNow(); ai.shutdown();
    }
}

...
/** OpenAI-compatible HTTP provider for Ollama and compatible servers. */
public final class NovaHttpAiProvider implements NovaAiProvider {
    private static final String TAG = "NovaHttpAI";
    ...
    public void chat(String endpoint, String apiKey, String model, JSONArray messages, Callback callback) {
        executor.execute(() -> {
            ...
            for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
                try {
                    ...
                    try {
                        ...
                        if (text.isEmpty()) throw new IllegalStateException("AI returned no text: " + compact(response));
            ...
            final String error = lastError + " Make sure the configured AI endpoint is reachable from the device.";
    ...
    private String extractText(JSONObject json) {
        ...
        if (choices != null && choices.length() > 0) {
            ...
            if (message != null && !message.optString("content", "").isEmpty()) return message.optString("content", "");
        }

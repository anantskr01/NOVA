...
public class GestureAccessibilityService extends AccessibilityService {
    ...
    @Override public void onAccessibilityEvent(AccessibilityEvent event){
        NovaFoodAgent agent=NovaFoodAgent.getActive();
        if(agent!=null)agent.onAccessibilityEvent();
    }
    ...
    public String getVisibleTextSummary(){
        AccessibilityNodeInfo root=getRootInActiveWindow();if(root==null)return"I cannot read the current screen. Accessibility window content may be unavailable.";
        StringBuilder out=new StringBuilder();collectText(root,out,0);if(out.length()==0)return"I cannot find readable text on the current screen.";return out.length()>1800?out.substring(0,1800):out.toString();
    }
    public String getUiSnapshot(){
        AccessibilityNodeInfo root=getRootInActiveWindow();if(root==null)return"No accessibility window is available.";
        StringBuilder out=new StringBuilder();collectUiSnapshot(root,out,0);if(out.length()==0)return"No readable UI elements found.";return out.length()>10000?out.substring(0,10000):out.toString();
    }
    private void collectUiSnapshot(AccessibilityNodeInfo node,StringBuilder out,int depth){
        ...
        CharSequence text=node.getText(),desc=node.getContentDescription();String label=text!=null&&text.length()>0?text.toString().trim():(desc!=null?desc.toString().trim():"");
        if(!label.isEmpty()||node.isClickable()||node.isEditable()){
            android.graphics.Rect b=new android.graphics.Rect();node.getBoundsInScreen(b);
    ...
    private void collectText(AccessibilityNodeInfo node,StringBuilder out,int depth){
        if(node==null||depth>25||out.length()>1800)return;CharSequence t=node.getText(),d=node.getContentDescription();if(t!=null&&t.length()>0){String v=t.toString().trim();if(!v.isEmpty())out.append(v).append('\n');}else if(d!=null&&d.length()>0){String v=d.toString().trim();if(!v.isEmpty())out.append(v).append('\n');}for(int i=0;i<node.getChildCount();i++)collectText(node.getChild(i),out,depth+1);
    }

    public boolean setTextOnAnyEditable(String text){
        if(text==null)return false;AccessibilityNodeInfo root=getRootInActiveWindow();if(root==null)return false;AccessibilityNodeInfo node=findEditable(root,0);return node!=null&&setText(node,text);
    }
    public boolean setTextOnBestEditable(String label,String text){
        AccessibilityNodeInfo root=getRootInActiveWindow();if(root==null)return false;AccessibilityNodeInfo target=findBestEditable(root,label==null?"":label.toLowerCase(),0);return target!=null&&setText(target,text);
    }
    private AccessibilityNodeInfo findEditable(AccessibilityNodeInfo n,int depth){
        if(n==null||depth>22)return null;if(n.isVisibleToUser()&&n.isEnabled()&&n.isEditable())return n;for(int i=0;i<n.getChildCount();i++){AccessibilityNodeInfo r=findEditable(n.getChild(i),depth+1);if(r!=null)return r;}return null;
    }
    private AccessibilityNodeInfo findBestEditable(AccessibilityNodeInfo n,String label,int depth){
        if(n==null||depth>22)return null;AccessibilityNodeInfo best=null;int bestScore=-1;for(int i=0;i<n.getChildCount();i++){AccessibilityNodeInfo r=findBestEditable(n.getChild(i),label,depth+1);int s=editableScore(r,label);if(s>bestScore){bestScore=s;best=r;}}int self=editableScore(n,label);if(self>bestScore)return n;return best;
    }
    private int editableScore(AccessibilityNodeInfo n,String label){
        if(n==null||!n.isVisibleToUser()||!n.isEnabled()||!n.isEditable())return-1;String h=(n.getHintText()==null?"":n.getHintText().toString()).toLowerCase();String t=(n.getText()==null?"":n.getText().toString()).toLowerCase();String d=(n.getContentDescription()==null?"":n.getContentDescription().toString()).toLowerCase();int s=10;if(!label.isEmpty()&&(h.contains(label)||t.contains(label)||d.contains(label)))s+=100;return s;
    }
    private boolean setText(AccessibilityNodeInfo node,String text){
        try{node.performAction(AccessibilityNodeInfo.ACTION_FOCUS);Bundle b=new Bundle();b.putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE,text);return node.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT,b);}catch(Exception e){Log.e(TAG,"SET TEXT ERROR",e);return false;}
    }
    ...
    private void collectClickableNodes(AccessibilityNodeInfo n,java.util.ArrayList<AccessibilityNodeInfo> out,int depth){
        if(n==null||depth>18||out.size()>=50)return;CharSequence t=n.getText(),d=n.getContentDescription();if(n.isClickable()&&n.isEnabled()&&((t!=null&&t.length()>0)||(d!=null&&d.length()>0)))out.add(n);for(int i=0;i<n.getChildCount();i++)collectClickableNodes(n.getChild(i),out,depth+1);
    }
    public boolean clickText(String requested){
        if(requested==null||requested.trim().isEmpty())return false;AccessibilityNodeInfo root=getRootInActiveWindow();if(root==null)return false;AccessibilityNodeInfo target=findBestTextNode(root,requested.trim().toLowerCase(),0);return target!=null&&clickNodeOrParent(target);
    }
    private AccessibilityNodeInfo findBestTextNode(AccessibilityNodeInfo node,String requested,int depth){
        if(node==null||depth>18)return null;AccessibilityNodeInfo best=null;int bestScore=-1;for(int i=0;i<node.getChildCount();i++){AccessibilityNodeInfo c=findBestTextNode(node.getChild(i),requested,depth+1);int s=scoreNode(c,requested);if(s>bestScore){bestScore=s;best=c;}}int self=scoreNode(node,requested);if(self>bestScore)return node;return best;
    }
    private int scoreNode(AccessibilityNodeInfo node,String requested){
        if(node==null||!node.isVisibleToUser())return-1;String v=node.getText()==null?"":node.getText().toString().trim().toLowerCase();String d=node.getContentDescription()==null?"":node.getContentDescription().toString().trim().toLowerCase();if(v.isEmpty()&&d.isEmpty())return-1;int s=0;if(v.equals(requested))s+=100;else if(v.contains(requested))s+=70;if(d.equals(requested))s+=95;else if(d.contains(requested))s+=65;if(node.isClickable())s+=35;if(node.isEnabled())s+=10;if(node.isFocusable())s+=5;return s;
    }
    private boolean clickNodeOrParent(AccessibilityNodeInfo target){
        if(target==null||!target.isVisibleToUser())return false;try{if(target.isEnabled()&&target.isClickable()&&target.performAction(AccessibilityNodeInfo.ACTION_CLICK))return true;AccessibilityNodeInfo p=target.getParent();int depth=0;while(p!=null&&depth++<6){if(p.isEnabled()&&p.isClickable()&&p.performAction(AccessibilityNodeInfo.ACTION_CLICK))return true;p=p.getParent();}}catch(Exception e){Log.e(TAG,"CLICK NODE ERROR",e);}return false;
    }

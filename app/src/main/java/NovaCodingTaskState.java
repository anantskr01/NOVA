...
public final class NovaCodingTaskState {
    ...
    public synchronized void observeTool(String tool, String value, String result, boolean verified) {
        ...
        if ("pc_search_text".equals(tool)) {
            phase = Phase.INSPECT;
        } else if ("pc_read_file".equals(tool)) {
            if (!path.isEmpty()) inspectedFiles.add(path);
            phase = Phase.PLAN;
        } else if ("pc_write_file".equals(tool)) {
            mutations++;
            if (!path.isEmpty()) modifiedFiles.add(path);
            phase = verified ? Phase.VERIFY_WRITE : Phase.RECOVER;
        } else if ("pc_git_diff".equals(tool)) {
            phase = verified ? Phase.REVIEW_DIFF : Phase.RECOVER;
        } else if ("pc_build".equals(tool)) {
            builds++;
            phase = verified ? Phase.BUILD : Phase.DIAGNOSE;
        } else if ("pc_run".equals(tool)) {
            tests++;
            phase = verified ? Phase.TEST : Phase.DIAGNOSE;
        } else if ("pc_git_status".equals(tool) || "pc_observe".equals(tool)) {
            phase = Phase.INSPECT;
        }
        if (!verified) {
            failures++;
            lastError = extractError(result);
    ...
    public synchronized JSONObject snapshot() {
        return new JSONObject()
                ...
                .put("lastEvidence", lastEvidence)
                .put("inspectedFiles", new JSONArray(inspectedFiles))
                .put("modifiedFiles", new JSONArray(modifiedFiles));
    }

...
/** On-device AI provider backed by NOVA's bundled local runtime. */
public final class NovaLocalAiProvider implements NovaAiProvider {
    ...
    public void chat(String endpoint, String apiKey, String model, JSONArray messages, Callback callback) {
        try {
            if (!runtime.tryChat(messages, new LocalModelRuntime.Callback() {
                ...
                callback.onError("Local AI runtime is not available");
            }
        } catch (Throwable t) {
    ...
    @Override public void shutdown() { runtime.shutdown(); }
}

...
public final class NovaAssistant {
    ...
    public NovaAssistant(Context context, Listener listener) {
        ...
        NovaPermissionGate.install(context instanceof Activity ? NovaPermissionGate.forActivity((Activity) context) : NovaPermissionGate.FAIL_CLOSED);
        ...
        tts = new TextToSpeech(this.context, result -> {
            if (result == TextToSpeech.SUCCESS) { try { tts.setLanguage(Locale.getDefault()); } catch (Exception ignored) { } }
        });
    }
    ...
    public void handle(String raw) {
        ...
        try {
            ...
            if (cancelTask.matches()) { String digits = cancelTask.group(1) != null ? cancelTask.group(1) : cancelTask.group(2); String id = "NOVA-T" + String.format(Locale.US, "%04d", Integer.parseInt(digits)); boolean cancelled = taskManager.cancel(id); say(cancelled ? "Cancelled " + id + "." : "I couldn't cancel " + id + ". It may already be finished or no longer exists."); return; }
    ...
    private boolean handleMemory(String c, String original) {
        if (c.equals("forget everything") || c.equals("clear memory") || c.equals("delete my memory")) { memory.clear(); context.getSharedPreferences("nova_user_memory", Context.MODE_PRIVATE).edit().clear().apply(); taskManager.cancelAll(); say("Local NOVA memory has been cleared."); return true; }
        if (c.startsWith("remember ")) { String note = original.substring(9).trim(); if (!note.isEmpty()) { int as = note.toLowerCase(Locale.ROOT).indexOf(" as "); if (as > 0 && as + 4 < note.length()) memory.rememberFact(note.substring(as + 4).trim(), note.substring(0, as).trim()); else { context.getSharedPreferences("nova_user_memory", Context.MODE_PRIVATE).edit().putString("note", note).apply(); memory.rememberFact("note", note); } say("I'll remember that locally on this tablet."); return true; } }
        if (containsAny(c, "what do you remember", "what do you know about me")) { String note = memory.factsSummary(); if (note.equals("No saved facts.")) note = context.getSharedPreferences("nova_user_memory", Context.MODE_PRIVATE).getString("note", note); say(note); return true; }
        return false;
    }
    ...
    private void search(String query) { status("WEB RESEARCH • " + query); web.search(query, new NovaWebTool.Callback() { @Override public void onResult(String text) { say(text); } @Override public void onError(String error) { if (!actions.execute("search", query)) { say("I couldn't open the search results."); return; } say("I opened the search results."); } }); }
    ...
    public void destroy() { if (tts != null) { try { tts.stop(); tts.shutdown(); } catch (Exception ignored) { } tts = null; } NovaPermissionGate.install(null); taskManager.shutdown(); brain.shutdown(); web.shutdown(); }
}

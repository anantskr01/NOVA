...
public final class NovaAssistant {
    ...
    public NovaAssistant(Context context, Listener listener) {
        ...
        NovaPermissionGate gate = context instanceof Activity
                ? new NovaConfirmationGate((Activity) context)
                : NovaPermissionGate.DENY_BY_DEFAULT;
    ...
    public void handle(String raw) {
        ...
        try {
            ...
            if (cancelTask.matches()) {
                ...
                boolean cancelled = taskManager.cancel(id); say(cancelled ? "Cancelled " + id + "." : "I couldn't cancel " + id + ". It may already be finished or no longer exists."); return;
            }
            ...
            if (numbered.matches()) {
                GestureAccessibilityService service = GestureAccessibilityService.getInstance(); int index = Integer.parseInt(numbered.group(1)); boolean ok = service != null && service.clickVisibleIndex(index); say(ok ? "Done." : "I couldn't activate that visible item."); return;
            }
            if (c.startsWith("open ") && openByName(command.substring(5).trim())) return;
            if (hasAiCore()) {
                int priority = parsePriority(c); String goal = stripPriorityPrefix(command, c); String id = taskManager.submit(goal, priority);
                if (id.isEmpty()) say("I couldn't add that task to NOVA's task manager."); else status("TASK " + id + " • ACCEPTED • P" + priority);
                return;
            }
